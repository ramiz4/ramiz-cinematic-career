...
test('offers and remembers iPhone installation guidance', async ({ page }, testInfo) => {
  ...
  await page.goto('./');
  await page.evaluate((key) => window.localStorage.removeItem(key), IOS_INSTALL_DISMISS_KEY);
  await page.reload();

  await page.locator('#system').evaluate((element) => element.scrollIntoView({ block: 'start' }));
